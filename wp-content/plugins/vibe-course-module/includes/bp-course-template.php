...
function bp_course_get_instructor_avatar_url($instructor_id){
	...
	if(function_exists('bp_core_fetch_avatar')){
		return bp_core_fetch_avatar(apply_filters('bp_course_get_instructor_avatar_url',array('item_id' => $instructor_id, 'type' => 'full', 'width' => 300, 'height' => 300, 'html'=>false)));
	}else{
		if(function_exists('vibe_get_option')){
	...
	if(bp_is_active('activity')){
		if(!empty($activity_id)){
			...
			$result = $wpdb->get_var($wpdb->prepare("
				...
				AND a.secondary_item_id = %d
				AND a.user_id = %d
				ORDER BY a.id DESC LIMIT 0,1",$quiz_id ,$user_id));
		}
	}
