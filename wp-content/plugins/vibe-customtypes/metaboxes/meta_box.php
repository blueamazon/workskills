...
class custom_add_meta_box {
	...
	function persistent_admin_scripts($hook){
		...
		if(in_array($hook,array('lms_page_lms-settings','post-new.php','post.php','lms_page_lms-stats'))){
			...
			wp_enqueue_media();
			wp_enqueue_style( 'meta_box_css', VIBE_PLUGIN_URL . '/vibe-customtypes/metaboxes/css/meta_box.css',array(),'3.8.5.1');
		}
		if($hook == 'term.php'){
			wp_enqueue_media();
		}
		...
		foreach ( $this->fields as $field) {
			...
			echo '<tr>
					...
					switch( $type ) {
						...
						break;
						case 'payments':
						echo '<ul id="instructor_payments"><li><strong>'.__('Instructor','vibe-customtypes').'</strong><span>'.__('Email','vibe-customtypes').'</span><span>'.__('Commission','vibe-customtypes').'</span><span>'.__('Currency','vibe-customtypes').'</span></li>';
						if(is_array($meta))
