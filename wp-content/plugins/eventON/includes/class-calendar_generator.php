...
class EVO_generator {
			...
			foreach($event_list_array as $event_):
				...
				// set how a single event would interact
					...
					// Calendar UX overrides
						...
						if($calendar_ux_val == '2'){
							...
							if( $EVO_Event->get_prop('evcal_lmlink')) 
								$event_permalink = $EVO_Event->get_prop('evcal_lmlink');
							
							// event external link
							if($EVO_Event->get_prop('evcal_exlink'))
								$event_permalink = $EVO_Event->get_prop('evcal_exlink');
						}
				...
				*/
				...
				/** CONSTRUCT the EVENT CARD	 **/
					...
					// Event link target
						$_eventInAttr['rest'][] =  $EVO_Event->check_yn('_evcal_exlink_target')? 'target="_blank" rel="noopener noreferrer"':null;
							...
							'{	"@context": "http://schema.org",
							  	...
							  	'.($location_name && $location_address? 
							  		',"location":{
										...
										"address":{
											"@type": "PostalAddress"
											"streetAddress":"'. str_replace("\,",",",$location_address).'"
										}
							  		}':''
							  	).'
							 }';
				...
				// if event is linking to external site
					if($event_ux_val == '2'){
						...
						// open in new window
						if($EVO_Event->check_yn('_evcal_exlink_target')){
	...
	// generate event data for all eventON events
		public function get_all_event_data($args = array()){
			...
			if(count($args['filters'])>0)	$wp_args = $this->apply_evo_filters_to_wp_argument( $wp_args ,$args['filters']);
